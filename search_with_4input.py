...
def try_config(args):
    ...
    n2, n3, n4, use_complements, restrict_functions = args
    cost = n2 * 2 + n3 * 3 + n4 * 4
    n_gates = n2 + n3 + n4
    ...
    try:
        result = solver._try_general_synthesis(n2, n3, n4, use_complements, restrict_functions)
        if result:
...
def main():
    print("=" * 60)
    print("BCD to 7-Segment Optimal Circuit Search (with 4-input gates)")
    print("=" * 60)
    print()
    print("Gates: AND, OR, XOR, XNOR, NAND, NOR (2, 3, and 4 input variants)")
    ...
    print(f"Searching {len(configs)} configurations from {min(c[0]*2+c[1]*3+c[2]*4 for c in configs)} to {max(c[0]*2+c[1]*3+c[2]*4 for c in configs)} inputs")
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if result is not None:
                        ...
                        if valid:
                            print(f"  {n2}x2 + {n3}x3 + {n4}x4 = {result_cost}: {status} (verified)")
                            ...
                            print(f"  {n2}x2 + {n3}x3 + {n4}x4 = {result_cost}: INVALID - {msg}")
                    else:
                        print(f"  {n2}x2 + {n3}x3 + {n4}x4 = {result_cost}: {status}")

                except Exception as e:
                    print(f"  {n2}x2 + {n3}x3 + {n4}x4: Error - {e}")
